...
class I18n
{
     ...
     *            IETF language tag, e.g. 'en' or 'en-US'.
     */
     ...
     * @param mixed $vars,...
     *            Values for placholders starting from %1, the first one (%1) is the
     *            numeral to test.
     ...
     *            UNIX timestamp or null for current timestamp.
     * @param string $style
     *            Which style to use ('short', 'medium', or 'long').
    ...
    public static function shortDate($timestamp = null)
    {
        ...
        } elseif (date('Y-m-d', strtotime('yesterday')) == $date) {
            return I18n::get('Yesterday %1', self::formatTime($timestamp));
        } elseif (date('Y-m-d', strtotime('tomorrow')) == $date) {
            return I18n::get('Tomorrow %1', self::formatTime($timestamp));
        } elseif ($cYear == date('Y', $timestamp)) {
    ...
    /**
     * A localized implementation of {@see strtotime}.
     * NOT YET IMPLEMENTED
     *
     * @param string $str
     *            A date/time string.
    ...
    /**
     * Natural language date parser with precision, e.g.
     * the string "june 2014"
