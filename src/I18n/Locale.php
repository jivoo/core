...
/**
 * A localization, e.g.
 * translation strings and date formats.
 *
...
class Locale
{
    ...
    /**
     * Messages in english and their local translation(s).
     *
     * @var array
     */
    ...
    /**
     * Default values for locale properties. The defaults are supposed to
     * represent a sort of international English using 24-hour clock and
     * ISO 8601 dates.
     *
     * @var string[]
     */
     ...
     *            Optional messages to create locale from.
     */
     ...
     * Non-default properties can be added using
     * {@see __set}.
     *
     ...
     * Non-default properties are added as messages
    ...
    public function __set($property, $value)
    {
        switch ($property) {
            case 'pluralForms':
                ...
                return;
            default:
                $this->messages['[Locale::' . $property . ']'] = array(
                    $value
                );
        }
    }
    ...
    /**
     * Unset a property.
     * For default properties, the value is reverted to the
     ...
     *            Translation string (or multiple
     ...
     *            Message in english.
     * @param mixed $vars,...
     *            Values for placeholders starting from %1.
    ...
    public function replacePlaceholders($message, $values = array())
    {
        ...
        foreach ($values as $value) {
            if (is_array($value)) {
                $message = preg_replace_callback('/%' . $i . '\{(.*?)\}\{(.*?)\}/', function ($matches) use ($value) {
                    $length = count($value);
                    $list = '';
                    for ($i = 0; $i < $length; $i ++) {
                        $list .= $value[$i];
            ...
            if ($message['msgid'] == '') {
                ...
                foreach ($properties as $property) {
                    ...
                    if (trim(strtolower($property)) == 'plural-forms') {
                        $l->pluralForms = $value;
                        break;
                    } elseif (trim(strtolower($property)) == 'language') {
